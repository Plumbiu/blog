---
title: JavaScript 面试题 (Copy)
date: 2024-02-03 12:09:00
updated: 2024-02-03 12:09:00
tags:
  - 面试
categories:
  - 面试
...
```javascript
javascript复制代码console.log(typeof 2);               // number
...
```javascript
javascript复制代码console.log(2 instanceof Number);                    // false
...
```javascript
javascript复制代码console.log((2).constructor === Number); // true
...
```javascript
javascript复制代码var a = Object.prototype.toString;
...
```javascript
javascript复制代码let n1 = 0.1, n2 = 0.2
...
```javascript
javascript复制代码function numberepsilon(arg1,arg2){                   
...
```javascript
javascript复制代码var a = new Boolean( false );
if (!a) {
...
```javascript
javascript复制代码var objToNumber = value => Number(value.valueOf().toString())
objToNumber([]) === 0
objToNumber({}) === NaN
```
...
```javascript
javascript复制代码3 == true // false, 3 转为number为3，true转为number为1
...
```javascript
javascript复制代码a.valueOf() // {}, 上面提到过，ToPrimitive默认type为number，所以先valueOf，结果还是个对象，下一步
...
```javascript
javascript复制代码console.log(...[1, 2, 3])
// 1 2 3
...
```javascript
javascript复制代码function add(x, y) {
  return x + y;
}
const numbers = [1, 2];
add(...numbers) // 3
...
```javascript
javascript复制代码const numbers = [9, 4, 7, 1];
Math.min(...numbers); // 1
Math.max(...numbers); // 9
```
...
通过把中间位留空，可以顺利地把数组第一位和最后一位的值赋给 a、c 两个变量： ![img](https://plumbiu.github.io/blogImg/a14ffbb3df2646a4a84f4a0c7d62d975~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)

**2）对象的解构** 对象解构比数组结构稍微复杂一些，也更显强大。在解构对象时，是以属性的名称为匹配条件，来提取想要的数据的。现在定义一个对象：

```javascript
javascript复制代码const stu = {
  name: 'Bob',
  age: 24
}
```
...
```javascript
javascript复制代码const { classes } = school
const { stu } = classes
const { name } = stu
name // 'Bob'
```
...
```javascript
javascript复制代码const { classes: { stu: { name } }} = school
...
```javascript
javascript复制代码function mutiple(...args) {
  let result = 1;
  for (var val of args) {
    result *= val;
  }
...
```javascript
javascript复制代码const father = 'xixi haha hehe'
...
```javascript
javascript复制代码const sourceCode = 'repeat for 3 times;'
...
```javascript
javascript复制代码var tmp = new Date();
...
```javascript
javascript复制代码var tmp = 'hello world';
...
```javascript
javascript复制代码getElementById // 按照 id 查询
...
**特点：** JavaScript 对象是通过引用来传递的，创建的每个新对象实体中并没有一份属于自己的原型副本。当修改原型时，与之相关的对象也会继承这一改变。 ![img](https://plumbiu.github.io/blogImg/c68fcad75ea54d62a9404aa02cafc65c~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp)
...
```javascript
javascript复制代码p.__proto__  // Person.prototype
...
```javascript
javascript复制代码function iterate(obj){
   var res=[];
   for(var key in obj){
...
```javascript
javascript复制代码for (var i = 1; i <= 5; i++) {
...
```javascript
javascript复制代码for (let i = 1; i <= 5; i++) {
  setTimeout(function timer() {
    console.log(i)
  }, i * 1000)
}
...
```javascript
javascript复制代码let a = 'Hello World!';
function first() {
...
```javascript
javascript复制代码Function.prototype.myCall = function(context) {
  // 判断调用对象
...
```javascript
javascript复制代码Function.prototype.myApply = function(context) {
  // 判断调用对象是否为函数
  if (typeof this !== "function") {
...
```javascript
javascript复制代码Function.prototype.myBind = function(context) {
  // 判断调用对象是否为函数
  if (typeof this !== "function") {
    throw new TypeError("Error");
  }
...
```javascript
javascript复制代码console.log('script start')	//1. 打印 script start
setTimeout(function(){
...
```javascript
javascript复制代码async function func1() {
    return 1
}
console.log(func1())
```
...
```javascript
javascript复制代码Promise.resolve(11).then(function(value){
...
```javascript
javascript复制代码function testPromise(ready) {
...
```javascript
javascript复制代码let promise = new Promise((resolve,reject)=>{
...
```javascript
javascript复制代码let promise1 = new Promise((resolve,reject)=>{
	setTimeout(()=>{
       reject(1);
	},2000)
});
...
```javascript
javascript复制代码server.listen(port)
  .then(function () {
    // ...
  })
  .finally(server.stop);
...
```javascript
javascript复制代码let fs = require('fs')
...
```javascript
javascript复制代码async function testAsy(){
   return 'hello world';
}
let result = testAsy(); 
console.log(result)
...
```javascript
javascript复制代码function getSomething() {
    return "something";
}
...
```javascript
javascript复制代码function testAsy(x){
...
```javascript
javascript复制代码async function doIt() {
    console.time("doIt");
    const time1 = 300;
...
```javascript
javascript复制代码async function fn(){
    try{
